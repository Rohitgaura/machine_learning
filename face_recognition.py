    ...
    def face_recog(self):
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in featuers:
                ...
                cursor.execute("select std_name from student where std_id="+str(id))
                n=cursor.fetchone()
                n = str(n)
                n=n[2:]
                n=n[-4::-1]
                ...
                cursor.execute("select roll from student where std_id="+str(id))
                r=cursor.fetchone()
                r = str(r)
                r = r[2:]
                r = r[-4::-1]
                ...
                cursor.execute("select std_id from student where std_id="+str(id))
                i=cursor.fetchone()
                i = str(i)
                i = i[2:]
                i = i[-4::-1]
                ...
                if confidence > 77:
                    cv2.putText(img,f"standard:{i}",(x,y-80),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),2)
                    cv2.putText(img,f"student name:{n}",(x,y-55),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),2)
